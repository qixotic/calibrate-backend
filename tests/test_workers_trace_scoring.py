...
def _create_agent(client, h, *, auto_score=False):
    created = client.post(
        "/agents",
...
def test_opted_in_trace_is_scored_end_to_end(client):
    h = _signup(client)
    agent_id, ev_uuid = _create_agent(client, h, auto_score=True)
...
def test_opted_out_trace_is_never_claimed(client):
    h = _signup(client)
    agent_id, _ = _create_agent(client, h, auto_score=False)
    trace_uuid = _ingest(client, h, agent_id)
    time.sleep(0.5)
    assert _run_for_trace(trace_uuid) is None
    with db.get_db_connection() as conn:
        scores = conn.execute(
...
def test_skipped_ingest_stays_skipped(client):
    h = _signup(client)
    agent_id, ev_uuid = _create_agent(client, h, auto_score=True)
    r = client.delete(f"/agents/{agent_id}/evaluators/{ev_uuid}", headers=h)
    assert r.status_code == 200, r.text
    trace_uuid = _ingest(client, h, agent_id)
    run = _run_for_trace(trace_uuid)
    assert run is not None
    assert run["status"] == "skipped"
    time.sleep(0.4)
    later = _run_for_trace(trace_uuid)
    assert later["status"] == "skipped"
    assert db.get_trace_scores(later["uuid"]) == []

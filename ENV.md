...
| **`TRACES_DATABASE_URL`**     | SQLAlchemy DSN for the traces store ([`src/traces/engine.py`](src/traces/engine.py)). Defaults to `sqlite:///${DB_ROOT_DIR}/traces.db`; set a Postgres DSN to move traces off SQLite. Migrations run at startup via Alembic ([`src/traces/migrate.py`](src/traces/migrate.py)). |
...
| **`DEFAULT_MAX_TRACES`**            | Default maximum stored traces per workspace when no org-specific `max_traces` limit exists ([`src/routers/org_limits.py`](src/routers/org_limits.py)); enforced by `POST /traces`. Default **`50000`**.                                                                                 |

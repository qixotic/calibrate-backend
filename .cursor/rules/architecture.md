...
| `apply_simulation_job_evaluator_enrichment()` | `routers/simulations.py` | **`simulation_jobs` only**: reads minimal UUID snapshot from `details.evaluators`; returns top-level `evaluators` with current names; mutates each `evaluation_results` dict to add `evaluator_uuid` from calibrate's echoed `evaluator_id`. Used by **`GET /simulations/run/{task_id}`**, **`POST …/abort`**, and **`GET /public/simulation-run/{token}`** (imported from `public.py`).                                                                                                                                                                                                                                                                      |
| `_enrich_test_results_with_evaluators()`      | `routers/agent_tests.py` | **`agent_test_jobs` only**: reads per-test snapshot from `details.evaluators_by_test_id`; converts each row's raw calibrate `judge_results` dict into `List[JudgeResult]` with `evaluator_uuid`, **current** DB name (refreshed via `get_evaluator()`), the snapshot-frozen `variable_values`, and (for rating evaluators) `scale_min`/`scale_max` from the pinned version's rubric. It uses calibrate's echoed `test_case_id` and per-judge `evaluator_id`. Idempotent — re-runs only refresh `name`. Used by **`GET /agent-tests/run/{task_id}`**, **`/agent-tests/agent/{uuid}/runs`**, **`/agent-tests/runs`**, and the public **`GET /public/test-run/{token}`** (imported from `public.py`). |
| `_enrich_model_results_with_evaluators()`     | `routers/agent_tests.py` | Benchmark wrapper: runs `_enrich_test_results_with_evaluators` on every `model_results[i].test_results`, then `_enrich_evaluator_summary` on each `model_results[i].evaluator_summary` (live `name`/`description` from `get_evaluator()` when `evaluator_uuid` is set). Used by **`GET /agent-tests/benchmark/{task_id}`**, **`GET /agent-tests/agent/{uuid}/runs`**, **`GET /agent-tests/runs`**, and the public **`GET /public/benchmark/{token}`** (same enrichment for list endpoints that embed `model_results`). |
...
| `_enrich_evaluator_summary()`                 | `routers/agent_tests.py` | Refreshes `name` / `description` on each `evaluator_summary` entry via `get_evaluator(evaluator_uuid)`; leaves numeric aggregates untouched. |
...
For these three families, **display** enrichment (evaluator `name` / `description` from the current DB) happens at read time on the response payload so renames flow through without a re-run. **Exception**: benchmark `evaluator_summary` **aggregate numbers** (`pass_rate`, `mean`, counts, etc.) are **persisted** from each model’s `metrics.json` when the worker finalizes that model; read paths only refresh labels on those stored rows, not recompute aggregates from `test_results`.
...
| `name`            | `str`                      | **Current** evaluator name from `get_evaluator()` when the evaluator row exists; otherwise the calibrate judge-results key.                                                                                                                                                                                                    |
...
3. Looks up each entry's evaluator via `get_evaluator(uuid)` to refresh `name` to the **current** DB value (latest rename wins).

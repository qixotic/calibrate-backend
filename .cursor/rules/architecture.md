...
│   ├── workers/trace_scoring.py  # Lifespan pool for trace scoring (independent of job queues)
│   └── routers/
...
| `trace_evaluations`                 | Durable trace-scoring **runs**. One row per scoring attempt (`pending` / `processing` / `completed` / `failed` / `skipped`); status is the source of truth. At most one **open** (`pending`/`processing`) run per trace (`ux_trace_eval_active`). A rescore or backfill is just another run — no `kind` column. Ingest creates the run; the lifespan pool in `workers/trace_scoring.py` claims and scores it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `trace_scores`                      | Persistent output of a run: one row per `(run_uuid, evaluator_uuid)`. Typed result columns — `match INTEGER` (binary 0/1) XOR `score REAL` (rating), CHECK-enforced — and `evaluator_version_id TEXT NOT NULL`. Same-version rescores are distinct runs and do not overwrite earlier rows.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
**Per-org default forks** (`db.py`): the seeded rows above are **hidden canonical templates** (`owner_user_id IS NULL AND org_uuid IS NULL`) that `get_all_evaluators()` **never returns to an org** (it filters `org_uuid = ?`, no `OR org_uuid IS NULL`). Instead every org gets its **own editable/deletable/renameable fork** of each default — provisioned at org creation (`_provision_default_evaluators_for_org`), lazily on the first `GET /evaluators` after a new default ships, and via a one-time deploy backfill — with the append-only `org_default_evaluators` receipt ledger enforcing the no-resurrection rule (a renamed/deleted fork is never re-created). A fork has `owner_user_id`+`org_uuid` set, `slug=NULL`, and `source_default_slug` for provenance; **the API's `is_default` is derived from `source_default_slug`** (OR `owner_user_id IS NULL`), **not `owner_user_id` alone** — so a fork reads as a default (UI groups it under the "Default" tab) yet stays fully editable, and only the hidden templates trip the `_owner_check` 403. Two further one-time deploy backfills re-point existing pivot links and finished-run job snapshots (`agent_test_jobs`/`simulation_jobs`/`jobs` JSON + `evaluator_runs`/`annotation_job_evaluators` columns) from template → org fork. Full detail in the root `CLAUDE.md` per-org-fork bullet.

**Evaluator name uniqueness**: Evaluator create, update, and duplicate requests validate `name` with `evaluator_name_exists()` before writing. Names must be unique within the **org's** evaluator namespace — its own non-deleted evaluators (its forks of the defaults plus from-scratch customs); the hidden seed templates live in their own namespace and don't collide. Different orgs can reuse the same name, but within one org a user cannot create or rename an evaluator to collide with another. Collisions return **409** from `routers/evaluators.py`. There is no database-level unique constraint for this because soft deletes make the namespace org-relative.

**Evaluator reserved names**: An evaluator's `name` becomes a `<name>`/`<name>_reasoning` column in bulk-CSV eval result rows, so a name matching a built-in column would clobber it. Create/update/duplicate requests reject a name equal (case-insensitively, after trimming) to any of **`RESERVED_EVALUATOR_NAMES`** (`{"name", "conversation_history"}` in `utils.py`) via a Pydantic `model_validator`, returning **422**. Backend enforcement of a rule the frontend also applies.

**Duplicate rubric scale points**: A `rating`/`binary` evaluator version's `output_config.scale` rejects two entries that repeat a label (`name`, compared case-insensitively/trimmed) or a `value`, returning **422** from the `EvaluatorVersionCreate` `model_validator` (covers both create and add-version). Validation lives on the *request* model, not on `OutputConfig` (which is reused by response models — validating there would 500 on any legacy row that already holds a dup).
...
Production traces are the platform's one **machine-paced** write path (customer backends POST one trace per agent turn). They live in `pense.db` like every other table: `traces` is created in `init_db()` and its storage functions (`create_trace`, `create_trace_with_eval_run`, `get_trace`, `get_traces_by_uuids`, `count_live_traces`, `list_traces`, `get_latest_trace_run_summaries`, `list_trace_scoring_runs`, `soft_delete_traces`) sit at the end of `db.py`. If traces ever outgrow SQLite, the whole database moves to Postgres together, not traces alone.

This is deliberately a **small surface**: send a trace, list them, open one, read its scoring history, delete some, turn some into tests. Text search, a `conversation_id` filter, delete-everything-matching, and a per-workspace configurable cap stay cut, parked on branch `traces-convert-to-tests-parked` (last full version at commit e844ea7). Do not re-add them without a reason.
...
- **Scoring schema + opt-in:** `agents.auto_score_traces INTEGER NOT NULL DEFAULT 0` (guarded `ALTER`, constant default — existing agents stay off). Set via `PUT /agents/{uuid}` (`auto_score_traces` optional; omit leaves it). The off→on flip is rejected (422) when zero linked evaluators are eligible; an already-on agent whose evaluators later all become ineligible stays on. Eligibility follows the agent's `interaction_type` (`conversation` → eval type `response` / `llm`, `general` → `general` / `llm-general`): matching type, live version present, no declared variables. JWT-only `GET /agents/{uuid}/trace-scoring-eligibility` returns the eligible/ineligible partition (`wrong_type_for_agent` | `no_live_version` | `declares_variables`). Disabling deletes that agent's `pending` `trace_evaluations` and leaves `processing` and terminal runs. Enabling does not backfill. Resolver: `trace_scoring.resolve_trace_scoring` / `resolve_scoring_plan` (no router imports). `POST /traces` uses `create_trace_with_eval_run`: resolve the plan outside the write lock, then insert the trace and its run in one `BEGIN IMMEDIATE` (deferred `BEGIN` can raise `SQLITE_BUSY` without honouring `busy_timeout`). Runnable runs are born `pending` with a non-NULL `criteria` snapshot; an empty or unsupported plan is a visible `skipped` run (`no_usable_evaluators` / `unsupported_interaction_type`). Opted-out ingest is still one trace insert and no run. `create_trace` is unchanged. After a pending run commits, ingest sets `trace_scoring_nudge` so idle workers wake; skipped/opted-out ingests do not. The FastAPI lifespan starts `workers.trace_scoring` (one worker, one subprocess, `claim_and_score_batch` on a thread, poll timeout as lease backstop) — independent of `MAX_CONCURRENT_JOBS_PER_ORG`. Shutdown stops new claims and waits for in-flight work; a run's lease is the backstop if the process dies. `trace_evaluations` is one durable run per scoring attempt; `criteria` is the JSON snapshot written at creation (`NULL` only on skipped runs). Indexes: `ux_trace_eval_active` (one open run per trace), `ix_trace_eval_claim` on `available_at` for open rows, `ix_trace_eval_agent_status` `(agent_id, status, completed_at)`, `ix_trace_eval_trace` `(trace_uuid, created_at DESC)`. `trace_scores` stores one result per `(run_uuid, evaluator_uuid)` with typed `match`/`score` columns (CHECK: exactly one) and `evaluator_version_id` NOT NULL; indexes `ix_trace_scores_trace` and `ix_trace_scores_org_eval`. A soft-deleted trace's open run is **not** cascaded here — claim/settle settles it `skipped`. JWT-only `GET /traces/{uuid}/scores` is the drill-down (full history, newest first); the list row is the latest run only (`get_latest_trace_run_summaries`, one query for the page). Row summary is the CLI pass conjunction, never a mixed-type average. Do not key anything on `message_id` / `conversation_id`.
...
- **Contract mirrors test creation** (see `routers/traces.py`): `input` is `tests.config.history` verbatim, and `output.tool_calls` uses the flat `{tool, arguments}` expected-tool-call shape (not OpenAI's nested `function` form), so the conversion needs no transformation.
- **Conversion** (`POST /traces/convert-to-tests`): one test per trace, at most `MAX_CONVERT_TRACES` (500) per request, JWT-only and deliberately **not** tagged `Public API`, so neither overlay file changes. `config.history` is the trace's `input` verbatim. For `tool_call` the recorded `output.tool_calls` become the expected calls (a trace with none is 400, and `accept_any_arguments` asserts the tool name only); for `response` the recorded reply is dropped, because the test re-runs the agent and judges the fresh one. Test name is the trace's `message_id`, or its uuid when that is null, made unique against the workspace's existing names with ` (2)`, ` (3)`, … so converting the same traces twice succeeds instead of colliding. Each created test links to the agent that produced its trace; an agent deleted since ingest is logged and skipped rather than failing a batch the user cannot retry. Linked evaluators go through the tests router's `_validate_evaluators`, plus two extra rejections (400): an evaluator whose live version declares `{{placeholder}}` variables (a conversion has no per-test place to supply values, so the prompt would reach the judge half-rendered), and an evaluator with no live version to run. `response` requires at least one evaluator, since it has no fallback judge; `tool_call` rejects them (400), because `_build_calibrate_config` skips any row whose type is not `response`/`conversation`, so a linked judge would be stored and never run.
...
- `GET /traces?limit=&offset=&agent_id=` - Paginated slim `TraceSummary` list in the shared `PaginatedResponse` envelope. JWT only. Filter and count run in SQL — never route this through `pagination.py`'s post-fetch `.apply()` helpers. Each row includes the **latest** scoring run only (`latest_run_status`, `passed`, `n_passed`, `n_total`), loaded in one batched query for the visible page (`get_latest_trace_run_summaries`). No run → those fields are null. A non-completed run contributes status only. `passed` is the CLI conjunction (binary `match`; rating at `scale_max`; AND across evaluators), never a mixed-type average, and never a blend of older runs.
- `GET /traces/{trace_uuid}` - Full `input`/`output`/`metadata`. JWT only.
- `GET /traces/{trace_uuid}/scores` - Full scoring history, newest run first. JWT only, not `Public API`. Each run carries status/timestamps/`error`; each evaluator result carries current (or last-stored) name, `evaluator_type`, `output_type`, rating bounds from the **pinned** version, raw `match`/`score`, `reasoning`, `evaluator_version_id`, and derived `passed`. Soft-deleted evaluators and historical versions still resolve.
...
- `POST /traces/convert-to-tests` - `{trace_ids}` (1 to 500), `type` (`response` | `tool_call`), `evaluators` (required for `response`, rejected for `tool_call`), `accept_any_arguments`. Returns `{created, test_uuids}` in the order the traces were given. JWT only, not `Public API`. 404 lists any `trace_ids` it could not find.
...
- `POST /datasets` - Create a new dataset (`name`, `dataset_type`: `stt`|`tts`). `name` must be unique within the workspace — create and rename go through `ensure_name_unique("datasets", …)` (whitelisted in `_UNIQUE_NAME_TABLES` + `idx_datasets_org_name_active` partial unique index), so a duplicate returns **409**, matching every other org-scoped entity (agents/tests/tools/personas/scenarios/simulations/annotation_tasks/annotators/evaluators). **One-time dedupe migration** (`DATASET_NAME_DEDUPE_MIGRATION`, gated by a `_schema_migrations` flag): since datasets never enforced uniqueness before this index, existing rows can collide, and building a UNIQUE index over duplicates raises `sqlite3.IntegrityError` — which the index-creation loop's `except sqlite3.OperationalError` does NOT catch, so it would crash `init_db` on startup. `_dedupe_active_dataset_names()` runs BEFORE the index loop, keeping the oldest active row's name and appending ` (2)`/` (3)`/… to newer collisions (skipping suffixes already taken in that org)
...
**Parameter-id uniqueness**: A tool parameter is identified by its `id`. A duplicate is only *representable* in the array-valued param lists — object children are stored as a JSON object keyed by the child's name (`properties: {city: {…}}`), where a duplicate key can't survive parsing (last-wins), so nested-level dupes are structurally impossible. `POST`/`PUT /tools` therefore enforce unique `id` (case-insensitively, after trimming) within the three arrays that carry entries: `config.parameters`, `config.webhook.queryParameters`, and `config.webhook.body.parameters` (descending each entry's nested `properties`/`items` schema for any deeper array). Returns **422** from the `ToolCreate`/`ToolUpdate` `model_validator` in `routers/tools.py`. The config is stored opaquely otherwise — this is the one part the backend parses. Backend enforcement of a rule the frontend also applies.

...
| **CI**               | GitHub Actions                      | Lint-free pipeline: install via `uv`, run pytest with coverage, upload to Codecov; **skipped** on push/PR when **only** docs/config/notebook paths change (`paths-ignore` in `tests.yml`: `*.md`, `*.yml`/`*.yaml`, `*.ipynb` and `**/*` variants); **`workflow_call`** (e.g. production deploy) always runs tests. **Production** release deploy (`deploy.yml`) gates on **`needs: test`**. Staging (`deploy-staging.yml`) is **`workflow_dispatch`** only and does **not** gate on tests. |

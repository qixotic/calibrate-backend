...
│   ├── shared_enums.py      # Closed test/agent/evaluator type sets and compatibility maps
│   ├── trace_scoring.py     # Trace-scoring eligibility/plan resolution and the claim→hydrate→invoke→settle engine
│   ├── trace_scoring_nudge.py    # Ingest-to-worker wakeup Event (reset per lifespan)
...
| `trace_eval_runs`                 | Durable trace-scoring **runs**. One row per scoring attempt (`TraceEvalRunStatus` in `trace_scoring.py`); status is the source of truth. At most one **open** (`OPEN_TRACE_EVAL_RUN_STATUSES`: pending/processing) run per trace (`ux_trace_eval_active`). A rescore or backfill is just another run — no `kind` column. Opted-in ingest writes `pending`/`skipped` rows; the lifespan pool (`workers.trace_scoring`) claims and scores them.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `trace_eval_scores`                      | Persistent output of a run: one row per `(run_uuid, evaluator_uuid)`. Typed result — `value NUMERIC NOT NULL` interpreted by `output_type` (`binary` 0/1 or `rating`), CHECK-enforced — and `evaluator_version_id TEXT NOT NULL`. Same-version rescores are distinct runs and do not overwrite earlier rows.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
Production traces are the platform's one **machine-paced** write path (customer backends POST one trace per agent turn). They live in `pense.db` like every other table: `traces` is created in `init_db()` and its storage functions (`create_trace`, `create_trace_with_eval_run`, `get_trace`, `get_traces_by_uuids`, `count_live_traces`, `list_traces`, `soft_delete_traces`) sit at the end of `db.py`. If traces ever outgrow SQLite, the whole database moves to Postgres together, not traces alone.

This is deliberately a **small surface**: send a trace, list them, open one, delete some, turn some into tests. Text search, a `conversation_id` filter, delete-everything-matching, and a per-workspace configurable cap stay cut, parked on branch `traces-convert-to-tests-parked` (last full version at commit e844ea7). Do not re-add them without a reason.
...
- **Scoring schema + opt-in:** `agents.auto_score_traces INTEGER NOT NULL DEFAULT 0` (guarded `ALTER`, constant default — existing agents stay off). Set via `PUT /agents/{uuid}` (`auto_score_traces` optional; omit leaves it). The off→on flip is rejected (422) when zero linked evaluators are eligible; an already-on agent whose evaluators later all become ineligible stays on. Eligibility follows the agent's `interaction_type` (`conversation` → eval type `response` / `llm`, `general` → `general` / `llm-general`): matching type, live version present, no declared variables. JWT-only `GET /agents/{uuid}/trace-scoring-eligibility` returns the eligible/ineligible partition (`wrong_type_for_agent` | `no_live_version` | `declares_variables`). Disabling deletes that agent's `pending` `trace_eval_runs` and leaves `processing` and terminal runs. Enabling does not backfill. Resolver: `db.resolve_live_evaluators` loads `(evaluator, live_version)` pairs; `trace_scoring.resolve_trace_scoring` partitions them (`TraceScoringResolution`); `as_plan` is the ingest snapshot. `POST /traces` uses `create_trace_with_eval_run`: resolve the plan outside the write lock, then insert the trace and its run in one `BEGIN IMMEDIATE` (deferred `BEGIN` can raise `SQLITE_BUSY` without honouring `busy_timeout`). Runnable runs are born `pending` with a non-NULL `scoring_plan` snapshot; an empty or unsupported plan is a visible `skipped` run (`TraceEvalSkipReason`: `no_usable_evaluators` / `unsupported_interaction_type`). Opted-out ingest is still one trace insert and no run. `create_trace` is unchanged. After a runnable (`pending`) run commits, ingest sets `trace_scoring_nudge` so idle workers wake; skipped/opted-out ingests do not. The FastAPI lifespan starts `workers.trace_scoring` (one worker, one subprocess, `claim_and_score_batch` on a thread, poll timeout as lease backstop) — independent of `MAX_CONCURRENT_JOBS_PER_ORG`. Shutdown stops new claims and waits for in-flight work; a run's lease is the backstop if the process dies. **Score reads are JWT-only** (not `Public API`): `GET /traces` rows carry the latest run's `latest_run_status`/`passed`/`n_passed`/`n_total` via one batched `db.get_latest_trace_run_summaries` per page (pass counts null unless that run is `completed`); `GET /traces/{uuid}/scores` returns the full run history newest-first with per-evaluator results (`db.list_trace_scoring_runs`). Hydration reads the score row's `output_type` and the pinned version's `output_config` (deleted evaluators/versions still render); `passed` follows the CLI rule (`trace_scoring.trace_evaluator_passed`: binary `value == 1`, rating `value == scale_max`) and the row summary is a conjunction, never a mixed-type average. `trace_eval_runs` is one durable run per scoring attempt (`TraceEvalRunStatus`); `scoring_plan` is the JSON snapshot written at creation (`NULL` only on skipped runs). Indexes: `ux_trace_eval_active` (one open run per trace), `ix_trace_eval_claim` on `available_at` for open rows, `ix_trace_eval_agent_status` `(agent_id, status, completed_at)`, `ix_trace_eval_trace` `(trace_uuid, created_at DESC)`. `trace_eval_scores` stores one result per `(run_uuid, evaluator_uuid)` with `value` + `output_type` (`binary` 0/1 or `rating`) and `evaluator_version_id` NOT NULL. A soft-deleted trace's open run is **not** cascaded here — claim/settle settles it `skipped`. Do not key anything on `message_id` / `conversation_id`.

...
- `POST /traces` - Ingest one production agent turn (`agent_id`, `message_id`, `conversation_id`, `input` history, `output`, optional `metadata`). `agent_id` is **required** and must resolve to a live agent in the caller's workspace — **404 `Agent not found`** (never 403, never 400) otherwise, checked *before* the idempotency lookup so a retry naming a bad agent fails the same way as the first call, and nothing is stored. Auth: `get_org_jwt_or_api_key` (workspace API key or JWT) — machine-to-machine, but deliberately **not** tagged `Public API` until that exposure is explicitly decided. Idempotent on `(org, message_id)`: retries return the stored row with `created: false`. 429 past the `max_traces` cap.
- `GET /traces?limit=&offset=&q=&conversation_id=&agent_id=` - Paginated slim `TraceSummary` list in the shared `PaginatedResponse` envelope. JWT only. Search/filter/count run in SQL in the traces store — never route this through `pagination.py`'s post-fetch `.apply()` helpers.
- `GET /traces/{trace_uuid}` - Full `input`/`output`/`metadata`. JWT only.
- `POST /traces/bulk-delete` - `{trace_ids | select_all, q?, conversation_id?, agent_id?}` (annotation-style bulk contract). `agent_id` bounds **both** modes — an agent-scoped caller must not reach another agent's traces through an explicit id list either. Soft delete; frees capacity and message IDs. JWT only — destructive, must never join the key-authed surface.

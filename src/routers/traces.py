...
TraceScoringStatus = Literal[
    "pending", "processing", "completed", "failed", "skipped"
]
_TRACE_RUN_STATUS_DESCRIPTION = (
...
class TraceSummary(BaseModel):
    ...
    created_at: str = Field(description="When the trace was created (ISO 8601 UTC)")
    latest_run_status: Optional[TraceScoringStatus] = Field(
        None,
    ...
    passed: Optional[bool] = Field(
        ...
        description=(
            ...
            "Binary passes on a true match. Rating passes at the top of its scale"
        ),
    )
...
class TraceScoreResult(BaseModel):
    ...
    match: Optional[bool] = Field(
        None, description="Pass/fail verdict, for binary evaluators"
    )
    score: Optional[float] = Field(
        None, description="Numeric score, for rating evaluators"
    )
    ...
    passed: bool = Field(
        description=(
            "Whether this evaluator passed. Binary passes on a true match. "
            "Rating passes at the top of its scale"
        )
    )
...
class TraceScoringRun(BaseModel):
    ...
    status: TraceScoringStatus = Field(description=_TRACE_RUN_STATUS_DESCRIPTION)
